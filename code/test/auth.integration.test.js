...
test('Existing username ', (done) => {
  request(app)
          ...
          .then((response) => {
              expect(response.status).toBe(400);
              expect(response.body).toStrictEqual({ error: "Username is already registered" });
              done();
          })
          .catch((err) => done(err));
});
test('Existing email ', (done) => {
  request(app)
          ...
          .then((response) => {
              expect(response.status).toBe(400);
              expect(response.body).toStrictEqual({ error: "Email is already registered" });
              done();
          })
          .catch((err) => done(err));
});
...
describe('login', () => { 
  ...
  test('existing user no scenario',  (done) => {
    request(app)
            ...
            .then((response) => {
                expect(response.status).toBe(400);
                expect(response.body).toStrictEqual({ error: "please you need to register" });
                done();
            })
            .catch((err) => done(err));
...
describe('logout', () => { 
  ...
  test('Nominal scenario',  (done) => {
    ...
    request(app)
            ...
            .then((response) => {
              expect(response.body).toStrictEqual({"data": {"message": "User logged out"}});
                expect(response.status).toBe(200);
